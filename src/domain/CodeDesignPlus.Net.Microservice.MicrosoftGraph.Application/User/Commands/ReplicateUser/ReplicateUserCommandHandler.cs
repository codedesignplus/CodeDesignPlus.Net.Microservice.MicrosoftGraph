...
public class ReplicateUserCommandHandler(IUserRepository repository, IPubSub pubsub) : IRequestHandler<ReplicateUserCommand>
{
    public async Task Handle(ReplicateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        var userAggregate = UserAggregate.Create(request.Id, request.IdIdentityProvider, request.IdentityProvider, request.FirstName, request.LastName, request.Email, request.Phone, request.DisplayName, request.DocumentNumber, request.DocumentType, null, null, true, request.IsActive);
