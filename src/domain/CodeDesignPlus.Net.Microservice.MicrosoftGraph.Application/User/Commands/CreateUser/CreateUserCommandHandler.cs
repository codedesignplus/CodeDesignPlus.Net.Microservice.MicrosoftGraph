...
public class CreateUserCommandHandler(IUserRepository repository, IMapper mapper, IIdentityServer identityServer, IPubSub pubSub, IVaultTransit vaultTransit, IOptions<VaultOptions> options) : IRequestHandler<CreateUserCommand>
{
    ...
    public async Task Handle(CreateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        var userAggregate = UserAggregate.Create(request.Id, idIdentityProvider, Domain.Enums.IdentityProvider.MicrosoftEntraExternalId, request.FirstName, request.LastName, request.Email, request.Phone, request.DisplayName, request.DocumentNumber, request.DocumentType, key, ciphertext, false, request.IsActive);
