...
vault kv put -mount=security-codedesignplus-keyvalue ms-microsoftgraph \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-microsoftgraph

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-microsoftgraph \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-microsoftgraph-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-microsoftgraph-mongo-role \
    db_name=db-ms-microsoftgraph \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-microsoftgraph" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-microsoftgraph-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-microsoftgraph-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-microsoftgraph-rabbitmq-role