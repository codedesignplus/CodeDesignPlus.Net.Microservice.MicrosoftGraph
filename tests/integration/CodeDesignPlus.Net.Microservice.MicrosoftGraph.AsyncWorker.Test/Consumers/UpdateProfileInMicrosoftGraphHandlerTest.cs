...
public class UpdateProfileInMicrosoftGraphHandlerTest(Server<Program> server) : ConsumerServerBase(server)
{
    ...
    public async Task Pusblish_Consumer_Success()
    {
        ...
        var userAggregate = UserAggregate.Create(Guid.NewGuid(), Guid.NewGuid(), Domain.Enums.IdentityProvider.MicrosoftEntraExternalId, "Joe", "Doe", "joee.doenew@fake.com", "3107545252", "Joe Doe", "1234567890", null, "key", "cipher", true, false);
        var domainEvent = new ProfileUpdatedDomainEvent(userAggregate.Id, "Joe New", "Doe New", "joe-new@fake.com", "3105631234", "Joe New Doe New", "1234567890", null, false, Domain.ValueObjects.ContactInfo.Create(
            "Street 123 New",
            "City New",
            "State New",
            "Country New",
