...
public class UpdateProfileInMicrosoftGraphHandlerTest(Server<Program> server) : ConsumerServerBase(server)
{
    ...
    public async Task Pusblish_Consumer_Success()
    {
        ...
        var userAggregate = UserAggregate.Create(Guid.NewGuid(), Guid.NewGuid(), Domain.Enums.IdentityProvider.MicrosoftEntraExternalId, "Joe", "Doe", "joee.doenew@fake.com", "3107545252", "Joe Doe", "1234567890", "key", "cipher", true, false);
