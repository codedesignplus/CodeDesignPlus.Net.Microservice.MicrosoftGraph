...
public class DeleteUserCommandHandlerTest
{
    ...
    public async Task Handle_UserNotFoundInIdentityServer_ThrowsUserNotExistInIdentityServerError()
    {
        // Arrange
        var request = new DeleteUserCommand(Guid.NewGuid());
        var userAggregate = UserAggregate.Create(request.Id, Guid.NewGuid(), Domain.Enums.IdentityProvider.MicrosoftEntraExternalId, "Joe", "Doe", "joee.doenew@fake.com", "3107545252", "Joe Doe", "1234567890", null, "key", "cipher", false, true);
