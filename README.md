...
5. Run the desired entry point:
   ...
   - For gRPC:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.MicrosoftGraph.gRpc
      ```
...
2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-microsoftgraph-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.MicrosoftGraph.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-microsoftgraph-grpc ms-microsoftgraph-grpc
   ```
...
3. Build the Docker image using the Dockerfile in the Worker entry point:
   ...
   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-microsoftgraph-worker ms-microsoftgraph-worker
   ```
